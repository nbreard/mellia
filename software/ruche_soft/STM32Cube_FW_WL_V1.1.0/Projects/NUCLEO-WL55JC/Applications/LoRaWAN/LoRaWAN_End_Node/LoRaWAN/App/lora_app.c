...
#include "sys_app.h"
#include "lora_app.h" //réglage timer
#include "stm32_seq.h"
...
#include "stm32_lpm.h"
#include "adc_if.h"
#include "sys_conf.h" //réglage debug et low power
...
static void SendTxData(void)
{
	...
	internal_Init();
	internal_Read(&sensor_data);
	Bme680Sensors_Init();
	Bme680Sensors_Read(&sensor_data);
	temperature_bme = (uint16_t)(sensor_data.temperature/100);
	temperature = (SYS_GetTemperatureLevel() >> 8);
	pressure    = (uint16_t)(sensor_data.pressure/100 );      /* in hPa / 10 */

	ADS1232Sensors_Init(&sensor_data);
	HAL_GPIO_WritePin(OPTO_GPIO_Port, OPTO_Pin, SET);
	HAL_Delay(5000);
	ADS1232_Read(&sensor_data);
	HAL_GPIO_WritePin(OPTO_GPIO_Port, OPTO_Pin, RESET);
	...
	CayenneLppAddTemperature(channel++, temperature_bme);
	CayenneLppAddTemperature(channel++, (uint16_t) sensor_data.internal_temp );
	CayenneLppAddRelativeHumidity(channel++, (uint16_t)(sensor_data.internal_hum));

	CayenneLppAddRelativeHumidity(channel++, (uint16_t)(sensor_data.humidity)/1000);

	CayenneLppAddAnalogInput(channel++, (int16_t) sensor_data.scale.weight);
